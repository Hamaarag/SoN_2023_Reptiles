...
First, let's get a clean start by removing all objects from the environment:
```{r clear environment}
rm(list = ls())
```

Next, load the required R packages:
```{r load packages}
# Setup----
library(tidyverse)
library(readxl)
...
```{r Load data}
# Reptile data:
reptiles.raw <- fread(here('data/raw/2023-04-19_all_rept_data.csv'))
reptiles <- copy(reptiles.raw)
reptiles

# Land use data:
land.use <- rast(here("data/GIS/LC_Madad_MigvanB_21v1.tif")) # see the ReadMe file to see where this file is
plot(land.use) # seems ok
land.use.legend <- read_excel(here("data/GIS/Categories4LC2020.xlsx"))

# Settlements data:
settlements <- st_read(here("data/GIS/SETL_bental_2021.shp")) # To calculate distance to nearest settlement
ggplot() + geom_sf(data = settlements) # seems ok
army.bases <- st_read(here("data/GIS/Army_camps_Harel_Dan/Fenced_15_11.shp")) # To calculate distance to nearest army base
...
missing.plot.names <- reptiles.raw[year != 2013 & is.na(point_name)] # omit the pilot year
...
reptiles[, .(no.of.lats = uniqueN(mean.lat), no.of.lons = uniqueN(mean.lon)), keyby = point_name][no.of.lats > 1 | no.of.lons > 1] # now there are no plots with multiple coordinates
...
study.design <- as.data.table(read_excel(here('Analysis/Study design.xlsx'))) # Import the study design - how many plots per site etc.
...
write_excel_csv(missing.excessive.plots, here('Analysis/Missing or excessive reptile plots.csv'))
...
zero.obs <- as.data.table(read_excel(here('Analysis/Missing reptile plots - Boaz comments.xlsx'), sheet = 'zero observations'))
str(zero.obs)
...
st_write(st_as_sf(excessive_plots[!is.na(longitude)], coords = c('longitude', 'latitude'), crs = 4326, remove = F),
         here('Analysis/Excessive reptile plots.shp'), append = F) # Export the excessive plots as an ESRI shapefile
st_write(st_as_sf(excessive_plots[!is.na(longitude)], coords = c('longitude', 'latitude'), crs = 4326, remove = F),
         here('Analysis/Excessive reptile plots.gpkg'), append = F) # Export the excessive plots as a geopackage
...
names(plot.distances.same.att) <- gsub('.x', '', names(plot.distances.same.att)) # remove the ".x" suffix
plot.distances.same.att[, ':=' (subunit.y = NULL, habitat.y = NULL, settlements.y = NULL, dunes.y = NULL)] # remove redundant columns
...
write_excel_csv(plot.distances.same.att, file = here('Analysis/Distances between plots in the same site.csv'))
```
...
write_excel_csv(plot.surveyed, here('Analysis/No. of plots surveyed breakdown 29.1.2024.csv'))
```
...
write_excel_csv(reptiles[, .(no.of.surveys = uniqueN(survey_ID)), keyby = .(unit, campaign, month(Date))],
                here('Analysis/No. of surveys per unit, campaign and calendar month.csv'))
write_excel_csv(reptiles[, .(no.of.surveys = uniqueN(survey_ID)), keyby = .(unit, month(Date))],
                here('Analysis/No. of surveys per unit and calendar month.csv'))
...
write_excel_csv(reptiles[, .(no.of.surveys = uniqueN(survey_ID)), keyby = .(unit, campaign, round(ITime))],
                here('Analysis/No. of surveys per unit, campaign and rounded hour.csv'))
write_excel_csv(reptiles[, .(no.of.surveys = uniqueN(survey_ID)), keyby = .(unit, round(ITime))],
                here('Analysis/No. of surveys per unit and rounded hour.csv'))
```

```{r Species and their traits}
species.list <- read_excel(here('data/raw/Reptile_species_traits.xlsx'), sheet = 'to_import') %>% as.data.table()
...
write_excel_csv(rept_cast, file = here('Output/Basic matrix - plots-campaign rows, species columns - individual counts.csv'))
```
...
write_excel_csv(rare.species, file = here('Output/Rare species to be removed from certrain analyses.csv'))
...
ggsave(here('Figures/Reptile species richness in the coastal sands - dotplot.png'), plot = coast.sp.rich.dotplot)
...
ggsave(here('Figures/Reptile species richness in the Inner sands - dotplot.png'), plot = Inner.sands.sp.rich.dotplot)
...
ggsave(here('Figures/Reptile Total Abundance in the Inner sands - dotplot.png'), plot = Inner.sands.abnd.dotplot)
```
...
write_excel_csv(inland.sands.plots, here("Output/Inland sands reptiles - distances to agriculture per plot.csv"))
write_excel_csv(inland.sands.plots[, .(mean.dist.to.agr = mean(LC_Madad_MigvanB_21v1), min.dist.to.agr = min(LC_Madad_MigvanB_21v1), max.dist.to.agr = max(LC_Madad_MigvanB_21v1), no.of.plots = uniqueN(point_name)), keyby = agriculture], here("Output/Inland sands reptiles - distances to agriculture summary statistics.csv"))
```
...
ggsave(here('Figures/Reptile species richness in the Loess - dotplot.png'), plot = loess.sp.rich.dotplot)
...
loess.abnd.dotplot <- ggplot(data = loess, aes(x = total.abundance, y = site)) +
  ...
  theme_bw() + theme(legend.position = 'bottom',
                     plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
loess.abnd.dotplot
ggsave(here('Figures/Reptile Total Abundance in the Loess - dotplot.png'), plot = loess.abnd.dotplot)
```
...
```{r rasterize army bases}
base.rast <- fasterize::fasterize(army.bases, raster = raster(land.use), field = NULL)
plot(base.rast, main = "Raster of army bases") # make sure the resultig raster makes sense
terra::writeRaster(base.rast, here("data/GIS/Army base raster.tiff"), overwrite = T) # export the raster
base.rast <- terra::rast(here("data/GIS/Army base raster.tiff")) # use terra to point to the geotiff file we just created to save memory
```
Rasterize the settlements:
```{r rasterize settlements}
sett.rast <- fasterize::fasterize(settlements, raster = raster(land.use), field = NULL)
plot(sett.rast, main = "Raster of settlements") # make sure the resulting raster makes sense
terra::writeRaster(sett.rast, here("data/GIS/Settlement raster 2021.tiff"), overwrite = T) # export the raster
sett.rast <- terra::rast(here("data/GIS/Settlement raster 2021.tiff")) # use terra to point to the geotiff file we just created to save memory
gc() # free up memory
```
Combine the army bases and settlements rasters:
```{r unite army bases and settlments rasters}
sett.base.rast <- terra::mosaic(sett.rast, base.rast)
plot(sett.base.rast, main = "Raster of settlements and army bases")
terra::writeRaster(sett.base.rast, here("data/GIS/Raster of settlements and army bases.tiff"), overwrite = T) # export the resulting raster as a file
sett.base.rast <- terra::rast(here("data/GIS/Raster of settlements and army bases.tiff")) # redefine the raster based on the file we just created
gc() # free up memory
```
Distance of plots to nearest settlement or army base:
```{r distance raster to nearest army base or settlement}
sett.dist <- distance(sett.base.rast)
plot(sett.dist, main = "Distance to the nearest settlement or army base") # This is more or less reasonable
sett.dist
writeRaster(sett.dist, here("Output/Raster of distances to the nearest settlement or army base.tif"), overwrite = T)
```
Find the coordinates of each plot
```{r Forest unit plots as SF objects}
...
Forest.plots <- st_as_sf(Forest.plots, coords = c("longitude", "latitude"), crs = 4326, remove = F) %>% st_transform(crs = 2039) # Convert to sf and reproject as Israel Transverse Mercator
...
```{r Forest unit distance to nearest army base or settlement}
Forest.plots <- as.data.table(cbind(Forest.plots, terra::extract(sett.dist, vect(Forest.plots))))
setnames(Forest.plots, old = "Settlement.raster.2021", new = "dist_nearest_sett") # rename for clarity
Forest.plots # Seems reasonable
```
Summarize and export the results:
```{r export distance to nearest settlement or army base}
Forest.plots[, .(mean.dist.to.sett = mean(dist_nearest_sett), min.dist.to.sett = min(dist_nearest_sett), max.dist.to.sett = max(dist_nearest_sett), N = uniqueN(point_name))]
write_excel_csv(Forest.plots, here("Output/Forest reptiles - distances to settlement or army base per plot.csv"))
write_excel_csv(Forest.plots[, .(mean.dist.to.sett = mean(dist_nearest_sett), min.dist.to.sett = min(dist_nearest_sett), max.dist.to.sett = max(dist_nearest_sett), no.of.plots = uniqueN(point_name))], here("Output/Forest reptiles - distances to settlement or army base summary statistics.csv"))
```
...
ggsave(here('Figures/Reptile species richness in the Forest - dotplot.png'), plot = Forest.sp.rich.dotplot, scale = 1.25)
...
Forest.abnd.dotplot <- ggplot(data = Forest, aes(x = total.abundance, y = site, color = subunit)) +
  ...
  theme_bw() + theme(legend.position = 'bottom',
                     plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
Forest.abnd.dotplot
ggsave(here('Figures/Reptile Total Abundance in the Forest - dotplot.png'), plot = Forest.abnd.dotplot, scale = 1.25)
...
ggsave(here('Figures/Reptile species richness in the sfar - dotplot.png'), plot = sfar.sp.rich.dotplot, scale = 1.25)
...
sfar.abnd.dotplot <- ggplot(data = sfar, aes(x = total.abundance, y = site, color = settlements,
  ...
  theme_bw() + theme(legend.position = 'bottom',
                     plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
sfar.abnd.dotplot
ggsave(here('Figures/Reptile Total Abundance in the sfar - dotplot.png'), plot = sfar.abnd.dotplot, scale = 1.25)
...
write_excel_csv(reptiles, here('data/processed/wrangled reptile data T0-T4.csv'))
write_excel_csv(reptiles[unit %like% "Plain"], here('data/processed/wrangled reptile data T0-T4 - Coastal Plain Dunes 18.7.24.csv'))
save(reptiles, coast, inland.sands, Forest, sfar, file = here('data/processed/wrangled reptile data T0-T4.Rdata'))
```
